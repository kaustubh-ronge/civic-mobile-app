...
// //   const pickImage = async () => {
// //     if (images.length >= 5) return Alert.alert("Limit Reached", "Max 5 images.");
// //     const result = await ImagePicker.launchImageLibraryAsync({
// //       mediaTypes: ['images'], 
// //       allowsMultipleSelection: true,
// //       selectionLimit: 5 - images.length,
// //       quality: 0.2, // Compresses images to stay under Vercel limits
// //     });
// //     if (!result.canceled) {
// //       setImages([...images, ...result.assets.map(a=>a.uri)].slice(0, 5));
// //     }
...
// //   const pickVideo = async () => {
// //     if (videos.length >= 2) return Alert.alert("Limit Reached", "Max 2 videos.");
// //     const result = await ImagePicker.launchImageLibraryAsync({
// //       mediaTypes: ['videos'], 
// //       allowsMultipleSelection: true,
// //       selectionLimit: 2 - videos.length,
...
// //       // 1. DIRECT MUX UPLOAD (Bypasses Vercel entirely for heavy files)
// //       const uploadedVideoIds: string[] = [];
      
// //       for (const videoUri of videos) {
// //         // Get secure ticket from Vercel
// //         const ticketRes = await fetch(`${API_BASE_URL}/mux-upload`);
// //         if (!ticketRes.ok) throw new Error("Failed to reach Mux upload endpoint.");
// //         const { uploadUrl, uploadId } = await ticketRes.json();

// //         // Safely upload heavy video from Phone -> Mux using FileSystem
...
// //           uploadedVideoIds.push(uploadId);
// //         } else {
// //           throw new Error("Mux rejected the video upload.");
// //         }
// //       }

// //       // 2. BUILD FORMDATA FOR DATABASE
// //       formData.append("title", title);
// //       formData.append("description", description);
// //       formData.append("cityId", selectedCity);
// //       formData.append("departmentId", selectedDept);
...
// //       // Append the Mux IDs we just generated so the backend knows about them
...
// //         setTitle(''); setDescription(''); setLocationData(null); setLocationQuery(''); setImages([]); setVideos([]); setTags([]);
// //       } else {
// //         Alert.alert("Server Rejected", data.error || "Failed to submit report.");
// //       }
// //     } catch (err: any) {
